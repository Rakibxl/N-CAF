...
namespace Architecture.BLL.Services.Implements.ClientProfile
{
    public class InsuranceInfoService : Repository<ProfInsuranceInfo>, IInsuranceInfoService
    {
        ...
        public async Task<ProfInsuranceInfo> GetById(int profileId, int insuranceInfoId)
        {
            var checkVal =await IsExistsAsync(x=>x.InsuranceInfoId == insuranceInfoId && x.ProfileId == profileId);
            if (checkVal)
            {
        ...
        public async Task<IEnumerable<ProfInsuranceInfo>> GetAll(int profileId)
        {
            IEnumerable<ProfInsuranceInfo> result;
              result=await GetAsync(x => x,x => x.ProfileId== profileId, null, x => x.Include(y => y.InsuranceType));
            return result;
